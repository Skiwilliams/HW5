...
/*Insert method in the Board class that will allow you to add a new player
   ...
   along with a desired (x,y) position into which it is to be initially
   placed. For successful insertion, (i) a player with the same ID should not
   already exist on the board; AND (ii) the specified insertion position on
   board should be currently unoccupied. A third condition would be to ensure
   the total number of players with this insertion should not exceed m. If
...
// removes player with given ID from both ids. returns bool indicating success
bool remove(int ID);

// removes player with given x,y pair from both Boards. returns bool
// indicating success
...
/*MoveTo
   ...
   player (ID) in its new position. If any player was removed in the process,
   the method should print a message to indicate which player was removed.
   prints when player eliminated Returns True on successfull move or displays
...
/*prints current board with vanity names*/
void printByVan();

/*checks coordinate pair to make sure it is in bounds of board*/
bool checkBounds(int x, int y);

/*checks move from x1, y1 to x2, y2, to make sure it is valid*/
